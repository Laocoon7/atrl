...
pub(crate) fn gen_tilemaps_for_maps(
    ...
    for mut map in q_map.iter_mut() {
        if map.terrain_storage_entity.is_none() {
        ...
        if map.feature_storage_entity.is_none() {
            if let Some(tileset) = tilesets.get_by_id(&map.feature_tileset_id) {
                info!("Tileset found! TilesetId: {}", &map.feature_tileset_id);
                let tilemap_entity = commands.spawn_empty().id();
        ...
        if map.item_storage_entity.is_none() {
            if let Some(tileset) = tilesets.get_by_id(&map.item_tileset_id) {
                info!("Tileset found! TilesetId: {}", &map.item_tileset_id);
                let tilemap_entity = commands.spawn_empty().id();
