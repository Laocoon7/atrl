...
impl<T> CellularAutomataBuilder<T> {
    ...
    fn count_neighbors(grid: &Grid<u32>, index: IVec2) -> u32 {
        let mut neighbors = 0;
        for y in -1..=1 {
            for x in -1..=1 {
                if x == 0 && y == 0 {
...
impl<T> MapArchitect<T> for CellularAutomataBuilder<T> {
    fn generate(&mut self, data: &mut MapGenData<T>) {
        ...
        for _ in 0..self.number_of_iterations {
            let mut new_tiles = data.output_grid.clone();
            rect.for_each(|index| {
                let neighbors = Self::count_neighbors(&data.output_grid, index);
