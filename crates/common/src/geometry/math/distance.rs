...
impl DistanceAlg {
    ...
    /// Provides a 2D distance between points, using the specified algorithm.
    pub fn distance2d(self, start: impl GridPoint, end: impl GridPoint) -> f32 {
        match self {
...
fn distance2d_pythagoras(start: impl GridPoint, end: impl GridPoint) -> f32 {
...
fn distance2d_pythagoras_squared(start: impl GridPoint, end: impl GridPoint) -> f32 {
...
fn distance2d_manhattan(start: impl GridPoint, end: impl GridPoint) -> f32 {
    let start = start.as_vec2();
    let end = end.as_vec2();
...
fn distance2d_diagonal(start: impl GridPoint, end: impl GridPoint) -> f32 {
    distance2d_diagonal_with_costs(start, end, CARDINAL_COST, DIAGONAL_COST)
}

// Calculates a diagonal distance
fn distance2d_diagonal_with_costs(start: impl GridPoint, end: impl GridPoint, d1: f32, d2: f32) -> f32 {
    let start = start.as_vec2();
    let end = end.as_vec2();
