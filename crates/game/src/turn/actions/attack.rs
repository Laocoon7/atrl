...
pub fn try_attack(entity: Entity, pos: Position, world: &mut World) -> Result<(), ActionType> {
    world.resource_scope(|world, map_manager: Mut<MapManager>| {
                ...
                map.get_actor(pos).map_or(Err(ActionType::Wait), |victim| {
                    ...
                    health_q.get_mut(world, victim).map_or_else(
                        ...
                        |(mut actor_health, name)| {
                            let before = format!("{}/{}", actor_health.current_hp, actor_health.max_hp);
                            actor_health.current_hp -= 1;
                            let after = format!("{}/{}", actor_health.current_hp, actor_health.max_hp);
